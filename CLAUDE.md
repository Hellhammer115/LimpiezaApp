...
- RLS on every table; catalog is client-read-only; `orders`/`order_items` are written ONLY by Edge Functions with the service role (admins additionally get `select` on every order). A cotización is an `orders` row with `paid_at IS NULL`. Clients never send amounts: `create-quote` recomputes all prices from the DB, and admins edit unpaid quotes only through `admin-orders` → `apply_quote_edit` (service-role RPC), which recomputes totals and clears any MP preference.
